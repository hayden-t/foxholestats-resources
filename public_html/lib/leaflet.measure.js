L.Control.Measure = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    this._createButton('', 'Measure Distance & Heading',
  ...
  _mouseMove: function (e) {
    ...
    if (this._tooltip) {
     ...
     var distance = this._map.distance(e.latlng, this._lastPoint)
          const anchor = {
		x: (this._lastPoint.lng),
		y: (256+this._lastPoint.lat)
	};
	const point = {
		x: (e.latlng.lng),
		y: (256+e.latlng.lat)
	};
     var angleDeg = (360-(Math.atan2(anchor.y - point.y, anchor.x - point.x) * 180 / Math.PI + 180))+90;
     if(angleDeg > 360)angleDeg = Math.abs(360 - angleDeg);//cheap hack
     angleDeg = angleDeg.toFixed(1);
     
      this._updateTooltipDistance(this._distance + distance, distance, angleDeg)
    }
  },
  ...
  _updateTooltipDistance: function (total, difference, angle) {
    if (!this._tooltip._icon) {
      return
    }
    var totalRound = this._formatDistance(total) + ' '+angle+'&#176;'

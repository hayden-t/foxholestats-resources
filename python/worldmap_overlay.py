...
greenColors = list(Color("#4cb72f").range_to(Color("#93d382"),5))#light to dark
blueColors = list(Color("#2f5dea").range_to(Color("#829df2"),5))
...
# WHERE regionId = 41
    ...
    for row in myresult:    
        ...
        for mapItem in _dynamic['mapItems']:     
            ...
            if mapItem['flags'] & 0x10 or mapItem['iconType'] == 70 or mapItem['iconType'] == 71 :
         ...
         #   print(mapItem)
            ...
            if mapItem['teamId'] == 'COLONIALS':
                ...
                _status += [3]
            elif mapItem['teamId'] == 'NONE' and 'wasTeam' in mapItem and mapItem['wasTeam'] == 'COLONIALS':
                _status += [4]
            elif mapItem['teamId'] == 'NONE' and 'wasTeam' in mapItem and mapItem['wasTeam'] == 'WARDENS':
                _status += [5]
            else:
                _status += [0]
        ...
        _status = pd.Series(_status) #shading
...
def drawOccupationMap():
        ...
        for key, region in enumerate(vor.regions):                                              
           ...
           if not -1 in region:                                                                 
              ...
              # print(key)                                                                      
               ...
               if drawDetails:                                                                  
                   plt.fill(x,y, color, linewidth=1,edgecolor='#000000')#debug draw with lines  
                   if debug:                                                                    
                       ax.plot(_data[:,0], _data[:,1], 'ko')#plot major marker points (towns)   
               elif drawVoro:                                                                            
                   plt.fill(x,y, facecolor=color,edgecolor=edgecolor, hatch=h, zorder=zorder )        
    ...
    for nuke in nuked:        
        ...
        if drawNuke: 
            if nuke['iconType'] == 70: #target
                ax.plot(cords[1], cords[0], marker='o' ,color='orange',markersize=15,markeredgecolor='white', zorder=100)
            else: #nuked
    ...
    #os.rename(imgSavePath + 'WorldMapControl.webp', imgSavePath + 'WorldMapControl_prev.webp')

    fig.savefig(imgSavePath + 'WorldMapControl.webp', transparent=True, dpi=100)
    ...
    #transparency
    im_rgba = Image.open(imgSavePath + 'WorldMapControl.webp')    
    blank = Image.new("RGBA", im_rgba.size)
    final = Image.blend(blank, im_rgba , 0.7)
   #im_rgba.putalpha(180)
    if doOpacity:        
        final.save(imgSavePath + 'WorldMapControl.webp', quality=25)
